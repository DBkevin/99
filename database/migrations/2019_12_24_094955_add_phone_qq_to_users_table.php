...
class AddPhoneQqToUsersTable extends Migration
{
    ...
    public function up()
    {
        Schema::table('users', function (Blueprint $table) {
            ...
            $table->decimal('price')->index()->default(0.00)->after('level')->comment('当前金额');
            $table->decimal('total_price')->after('price')->default(0.00)->comment('累计充值金额');
            $table->decimal('payment')->after('total_price')->default(0.00)->comment('累计消费金额');
            $table->decimal('tixian_price')->after('payment')->default(0.00)->comment('累计提现金额');
